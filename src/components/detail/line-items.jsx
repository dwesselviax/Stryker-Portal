  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[#D4D4D4]">
      <table className="w-full">
        ...
        <tbody>
          {items.map((item, i) => (
            <tr key={item.biiId || i} className="border-t border-[#D4D4D4]">
              <td className="px-4 py-3 text-sm text-black">{item.biiId || item.id}</td>
              <td className="px-4 py-3 text-sm text-black">{item.biiName || item.name}</td>
              ...
              <td className="px-4 py-3 text-right text-sm font-semibold text-black" style={{ fontFamily: 'var(--font-heading)' }}>{formatCurrency(item.total || item.unitPrice * item.quantity)}</td>
            </tr>
          ))}
        </tbody>
      </table>
